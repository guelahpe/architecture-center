...
6. [Obtain an SSL Certificate for AD FS][adfs_certificates] and install this certificate on the AD FS VMs. Note that you can connect to them through the jump box. The IP addresses are <em>10.0.5.4</em> and <em>10.0.5.5</em>. The default username is <em>contoso\testuser</em> with password <em>AweSome<xref href="PW" data-throw-if-not-resolved="False" data-raw-source="@PW"></xref></em>.
